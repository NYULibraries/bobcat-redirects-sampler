...
export class AbstractServiceSampler {
    ...
    getServiceResponseSampleFilePathRelative( key ) {
        return path.join( this.testCaseGroup, this.serviceName, `${key}.html` );
    }

    getServiceResponseScreenshotFilePathRelative( key ) {
        return path.join( this.testCaseGroup, this.serviceName, `${key}.png` );
    }
